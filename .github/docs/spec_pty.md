...
- **Child stdin must not be closed before launch completes.** Closing the ConPTY input pipe too early can yield `STATUS_CONTROL_C_EXIT` (0xC000013A) rather than normal command output. `SendEof()` only sets a flag; the pipe is closed on the first `WaitForExitAsync` poll (reusing the existing wait interval) or in `CloseTransport` on exit/dispose — not via a fixed `Thread.Sleep` in `SendEof()`.
- **Stdin EOF must be signaled for one-shot input.** After writing bytes, call `SendEof()` before waiting. Windows eventually closes the ConPTY input pipe (deferred close); Unix writes EOT (`0x04`, Ctrl-D) because PTY master fds cannot be half-closed — EOT is only EOF in canonical terminal mode. Do not close the input pipe via `FileStream` disposal on the write handle; use `WriteFile` and an explicit `SendEof()`.
